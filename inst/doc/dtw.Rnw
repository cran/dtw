...
\author{Toni Giorgino\\University of Pavia}
...
\Address{
  ...
  via Ferrata 1\\
  I-27100, Pavia, Italy\\
  E-mail: \email{toni.giorgino@gmail.com}\\
  URL: \url{http://www.labmedinfo.org/people/cv/giorgino.htm} \\

  Currently with:\\
  Computational Biochemistry and Biophysics Laboratory\\
  Research Group on Biomedical Informatics (GRIB-IMIM)\\
  Parc de Recerca Biom\`edica de Barcelona\\
  c/ Dr.~Aiguader, 88\\
...
\begin{figure}
  \centering
  \includegraphics[width=.95\textwidth]{object-diagram}
...
\begin{figure}
  \centering
  \includegraphics[width=.9\textwidth]{plot-conventions}
...
\begin{figure}
  ...
  \includegraphics[width=.7\textwidth]{alignMVM}
  \includegraphics[width=.27\textwidth]{mvmSP}
...
\caption{The local distance matrix for the exercise in
  ...
  cumulative distance matrix (right) under the symmetric step
